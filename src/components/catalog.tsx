...
export function Catalog({ categories, products }: { categories: Category[]; products: Product[] }) {
  ...
  useEffect(() => {
    ...
    if (pendingCategory.current) {
      if (nextCategory === pendingCategory.current) pendingCategory.current = null;
      else return;
    }
  ...
  }, [searchParams, category, scrollToCategory]);
  useEffect(() => {
  ...
  return <section className="catalog" id="shop">
    ...
    <div className="catalogResults" id={anchorFor(category)}>
