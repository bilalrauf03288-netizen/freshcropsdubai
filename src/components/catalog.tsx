...
export function Catalog({ categories, products }: { categories: Category[]; products: Product[] }) {
  ...
  return <section className="catalog" id="shop">
    ...
    <div className="categoryRail" aria-label="Product categories"><button className={category === "all" ? "active" : ""} onClick={() => setCategory("all")}><span className="allCategoryThumb" aria-hidden="true">{categories.slice(0, 4).map((item) => <Image src={item.image} width={96} height={96} alt="" key={item.id} loading="eager" quality={100} sizes="64px" />)}</span><span>All Products</span></button>{categories.map((item) => <button className={category === item.id ? "active" : ""} key={item.id} onClick={() => setCategory(item.id)}><span className="categoryImage" aria-hidden="true"><Image src={item.image} width={256} height={256} alt="" loading="eager" quality={100} sizes="(max-width: 620px) 112px, 128px" /></span><span>{item.name}</span></button>)}</div>
